...
`FileSystem` обектите трябва да имат следните методи:
  ...
  * `get_node(path)` - връща обект(файл или директория) намиращ се на определения път. Ако няма такъв хвърля `NodeDoesNotExist`.
  ...
  * `remove(path, directory=False, force=True)` - изтрива това, към което сочи въпросния път.
    * Ако `path` сочи към директория, то `directory` трябва да е изрично указан като `True`, в противен случай да се хвърля изключение от тип `NonexcplicitDirectoryDeletionError`.
